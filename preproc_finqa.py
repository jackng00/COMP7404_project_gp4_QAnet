...
def process_file(filename, data_type, word_counter, char_counter):
    ...
    with open(filename, "r") as fh:
        ...
        for item in source:
            ...
            for text in item['pre_text']:
                context += text.replace("''", '" ').replace("``", '" ')
                context += ' '
            for text in item['post_text']:
                context += text.replace("''", '" ').replace("``", '" ')
                context += ' '
            # if len(context)>2000:
            #     continue
            ...
            #print(spans)
            get_count = 0
            for idx, ans in enumerate(item['annotation']['turn_program_ori']):
                y1s, y2s = [], []
                answer_texts = []
                
                if numeric_regex.match(ans) and 'and' not in item['annotation']['dialogue_break_ori'][idx] and item['annotation']['dialogue_break_ori'][idx] != 'n/a' and get_count == 0 :
                    #total += 1
                    ques = item['annotation']['dialogue_break_ori'][idx].replace("''", '" ').replace("``", '" ')
                    ...
                    answer_span = []
                    # if len(context)>2000 :
                    #     context= context[answer_start-1000:answer_start+1000]
                    # context_tokens = word_tokenize(context)
                    # context_chars = [list(token) for token in context_tokens]
                    # spans = convert_idx(context, context_tokens)
                    ...
                    #print(answer_span,answer_texts)
                    ...
                    eval_examples[str(total)] = {
                        "context": context, "spans": spans, "answers": answer_texts, "uuid": uuid.uuid4().hex}
...
def preproc_finqa(config):
    word_counter, char_counter = Counter(), Counter()
    train_examples, train_eval = process_file(config.train_file_finqa, "train", word_counter, char_counter)
    dev_examples, dev_eval = process_file(config.dev_file_finqa, "dev", word_counter, char_counter)
