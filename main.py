...
from preproc_finqa import preproc_finqa
...
def train_entry(config):
    ...
    elif config.model == "finqa":
        fn = os.path.join(config.pre_dir, "model.pt")
        model = torch.load(fn)
    ...
    for iter in range(config.num_epoch):
        ...
        if (dev_f1+dev_em)/2 > best_score:
            best_score = (dev_f1+dev_em)/2
            if config.model == "qanet":
                fn = os.path.join(config.pre_dir, "model.pt")
                torch.save(model, fn)
            elif config.model == "finqa":
